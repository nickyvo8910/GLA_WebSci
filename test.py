...
def restProbe(callLeft):
    ...
    try:
        while(callLeft>0):
            myrestProbe = api.search(q='mariahcarey',languages = ['en'],count=100)
...
def streamGeo():
    ...
    myStream = tweepy.Stream(auth = api.auth, listener=myStreamListener)
    # myStream.track(languages = ['en'],track=['mariahcarey'],async=True)
    myStream.filter(languages = ['en'],locations=[-4.393201,55.781277,-4.071717,55.929638],async=True)

_RUNNING_TIME = 5*60

if __name__ == '__main__':
    # Start as a process
    p = multiprocessing.Process(target=streamBasic, name="streamBasic" )#args=(10,))
    p.start()
    p2 = multiprocessing.Process(target=streamEnhanced, name="streamEnhanced" )#args=(10,))
    p2.start()
    p3 = multiprocessing.Process(target=restProbe, name="restProbe" ,args=(_RUNNING_TIME,))
    p3.start()
    p4 = multiprocessing.Process(target=streamGeo, name="streamGeo" )#args=(10,))
    p4.start()
    ...
    # If thread is active
    if p.is_alive():
        print("{} {} {}".format("streamBasic has been running for ", _RUNNING_TIME,"... let's kill it..."))

        # Terminate foo
        p.terminate()
        p.join()
    if p2.is_alive():
        print("{} {} {}".format("streamEnhanced has been running for ", _RUNNING_TIME,"... let's kill it..."))

        # Terminate foo
        p2.terminate()
        p2.join()
    if p3.is_alive():
        print("{} {} {}".format("restProbe has been running for ", _RUNNING_TIME,"... let's kill it..."))

        # Terminate foo
        p3.terminate()
        p3.join()
    if p4.is_alive():
        print("{} {} {}".format("streamGeo has been running for ", _RUNNING_TIME,"... let's kill it..."))
